...
function AreaChart({ years, petrolYears, animated }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{width:"100%",height:"auto",display:"block"}}>
      ...
      {years.map((y,i)=>i%2===0&&(
        <text key={y.year} x={toX(i)} y={H-6} textAnchor="middle" fill="rgba(255,255,255,0.28)" fontSize="8.5" fontFamily="'IBM Plex Mono',monospace">{y.year}</text>
      ))}
...
function Earth({ hot, size=90 }) {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 90 90" style={{animation:hot?"pHot 3s ease-in-out infinite":"bCool 4s ease-in-out infinite"}}>
      <defs>
      ...
      {hot&&<><circle cx="45" cy="45" r="39" fill="none" stroke="rgba(255,100,0,0.25)" strokeWidth="4"/><circle cx="45" cy="45" r="44" fill="none" stroke="rgba(255,50,0,0.12)" strokeWidth="6"/></>}
    </svg>
  );
}
  ...
  return (
    <div>
      <div style={{display:"flex",gap:".35rem",flexWrap:"wrap",marginBottom:"1rem"}}>
        {years.map((yr,i)=>(
          <button key={yr.year}
            onClick={()=>{setIdx(i);clearInterval(ref.current);}}
...
export default function ClimateProjection() {
  ...
  // Accept flat state {emissions_kg, distance_km, vehicle} from wallet/TravelLogImpact
  // OR nested state.trip from TravelLogger — normalise to one shape
  ...
  const doFetch = useCallback(async()=>{
    ...
    try{
      ...
      if(tripData?.emissions_kg != null && tripData?.distance_km != null){
        // Have specific trip data — use POST
        res = await fetch(`${API}/impact/projection`,{
          method:"POST",
        ...
        // No trip data — derive from wallet (standalone visit from /wallet header)
        res = await fetch(`${API}/impact/projection/${userId}`);
      }
  ...
  useEffect(()=>{if(proj){const t=setTimeout(()=>setReady(true),250);return()=>clearTimeout(t);}},[proj]);

  const years      = proj?.years??[];
  const last       = years[years.length-1];
  const userTotal  = last?.cumulative_co2_kg??0;
  const freq       = tripData?(tripData.distance_km>50?1:5):5;
  const petrolAnn  = tripData?(tripData.distance_km*PETROL_G_PER_KM/1000)*freq*52:(userTotal/10)*2.4;
        ...
        <main style={S.main}>
          ...
          {error&&(
            <div style={{textAlign:"center",padding:"4rem 2rem",color:"#ff6b6b"}}>
              <p>⚠ {error}</p>
              <button onClick={doFetch} style={{marginTop:"1rem",background:"#00C853",color:"#001a0d",border:"none",padding:".6rem 1.2rem",borderRadius:"8px",fontWeight:700,cursor:"pointer"}}>Retry</button>
            </div>
          )}
          ...
          {!loading&&!error&&proj&&(<>
            ...
            <div style={{...S.card,display:"flex",alignItems:"center",justifyContent:"center",gap:"2rem",flexWrap:"wrap",padding:"2.25rem 1.75rem",background:"radial-gradient(ellipse 80% 80% at 50% 50%,rgba(0,200,83,0.04),#111412)",position:"relative",overflow:"hidden"}}>
              ...
              {[
                {label:"Petrol baseline",hot:true,co2:petrolTotal,cls:"globe-lbl--b",col:"#FF1744"},
                null,
                {label:"Your vehicle",hot:false,co2:userTotal,cls:"globe-lbl--a",col:"#00C853"},
                ...
                <div key={s.label} style={{background:s.bg,border:`1px solid ${s.bc}`,borderRadius:"10px",padding:"1.25rem"}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:"1rem",marginBottom:"1rem"}}>
                    <div style={{flex:1}}>
                      ...
                      <div style={{fontSize:".63rem",color:"rgba(221,232,222,0.42)",marginTop:".05rem"}}>total emitted</div>
                    </div>
                    ...
                    <div style={{flex:1}}>
                      ...
                      <div style={{fontSize:".63rem",color:"rgba(221,232,222,0.42)",marginTop:".05rem"}}>to offset</div>
                    </div>
                  </div>
